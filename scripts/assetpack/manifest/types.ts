...
export interface ManifestOptions extends PluginOptions {
  /**
   * The output location for the generated typescript definitions. If unset, does not generate
   *
   * @default undefined
   */
  typesOutput?: string
  ...
  /**
   * The naming convention for asset bundles in the manifest. - 'short': Uses the directory name containing the `{m}`
   * tag. - 'relative': Uses the full relative path to the directory containing the `{m}` tag.
   *
   * @default 'short'
   */
  nameStyle?: 'short' | 'relative'
  /**
   * If true, all tags are outputted in the `data.tags` field of the manifest. If false, only internal tags are
   * outputted to `data.tags`, while other tags are outputted directly to the `data` field.
   *
   * @example
   *   ```json
   *   {
