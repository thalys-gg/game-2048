...
- **`scripts/`** — tooling, runs under Bun directly *and* under Node when Vite loads it (keep code runtime-agnostic on the vite-plugin import chain; `Bun.*` is only safe in files executed exclusively by Bun, e.g. `scripts/bun/git/hooks/`).
